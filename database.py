...
class DataManager:
    def __init__(self, data_dir):
        ...
        # Файл, который создает твой PHP скрипт (источник)
        self.leads_file = os.path.join(data_dir, "leads.txt")
        
        # Временный файл: связывает UserID c Yclid пока юзер решает капчу
        self.temp_file = os.path.join(data_dir, "waiting_captcha.txt") 
        
        # Финальный файл: очередь подтвержденных конверсий для отправки в Яндекс
        self.verified_file = os.path.join(data_dir, "to_send_yandex.txt") 

    def find_yclid_by_hash(self, invite_hash):
        """Ищет хеш ссылки в едином файле leads.txt"""
            ...
            with open(self.leads_file, 'r', encoding='utf-8') as f:
                ...
                for line in f:
                    ...
                    try:
                        ...
                        # PHP должен писать JSON вида: {"hash": "...", "yclid": "..."}
    ...
    def save_temp_link(self, user_id, yclid):
        """Запоминаем, что этот юзер (ID) пришел с этим yclid"""
        try:
    ...
    def get_yclid_from_temp(self, user_id):
        """Ищем yclid по юзеру после прохождения капчи"""
        ...
        try:
            ...
            # Ищем совпадение по UserID
            for line in lines:
                if str(user_id) in line:
                    parts = line.strip().split('|')
                    if len(parts) >= 2:
                        target_yclid = parts[1]
                        # Не прерываем (берем последний, если кликал много раз)
        except:
            return None
    ...
    def save_verified_user(self, user_id, yclid):
        """Финальная запись: этот YCLID можно слать в Метрику"""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # Формат: USER_ID | YCLID | STATUS | TIME
