...
## Где создаются ссылки? Ссылки должен создавать твой PHP-скрипт на сайте. Он делает это от имени этого же бота.

Схема такая:

PHP (Сайт): Использует Токен Бота, чтобы сказать Телеграму: "Дай мне ссылку!". Записывает её в файл.

Python (Сервер): Использует Токен Бота, чтобы увидеть: "Кто-то зашел по ссылке". Читает файл.
